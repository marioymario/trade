...
from files.research.entry_quality_event_robustness import (
    DEFAULT_BOOTSTRAP_REPS,
    DEFAULT_PERMUTATION_REPS,
    DEFAULT_RANDOM_SEED,
    build_entry_quality_event_robustness,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run post-selection event-cluster robustness analysis "
            "for the Universe V1 PRIMARY_COMPARABLE population."
        )
    )
    ...
    parser.add_argument(
        "--permutation-reps",
        type=int,
        default=DEFAULT_PERMUTATION_REPS,
    )
    ...
    manifest = build_entry_quality_event_robustness(
